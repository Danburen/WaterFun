  ...
  <el-card shadow="never">
    <template #header>
      <div class="wf-flex-between">
        <h3 class="wf-module-header">{{ $t('notification.system') }}</h3>
    ...
    <div class="system-list" @scroll.passive="onScroll">
      <el-card
      ...
      >
        <div class="wf-notification-header">
          <div class="wf-title-wrap">
            ...
            <span class="wf-notification-title">{{ item.title || '系统通知' }}</span>
          </div>
          <span class="wf-notification-time">{{ formatTime(item.createdAt) }}</span>
        </div>
        <div class="wf-notification-content">{{ item.content?.displayText || item.content }}</div>
      </el-card>
...
const notifications = computed(() => notificationStore.getNotifications('system'))
const loading = computed(() => notificationStore.isLoading('system'))
const hasNext = computed(() => notificationStore.hasMore('system'))
...
const load = (reset = false) => {
  notificationStore.fetchNotifications('system', reset)
}
...
<style scoped>
.system-list { height: calc(100vh - 240px); min-height: 460px; overflow-y: auto; padding-right: 8px; }
.notification-content { white-space: pre-wrap; word-break: break-word; }
.skeleton { margin-top: 12px; }
@media (max-width: 768px) { .system-list { height: auto; min-height: 360px; } }
</style>
