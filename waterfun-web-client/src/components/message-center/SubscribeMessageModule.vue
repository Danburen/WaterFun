  ...
  <el-card shadow="never">
    <template #header>
      <div class="wf-flex-between">
        <h3 class="wf-module-header">{{ $t('notification.subscribe') }}</h3>
      ...
      >
        <div class="wf-notification-header">
          <div class="wf-title-wrap">
            ...
            <span class="wf-notification-title">{{ item.title || '订阅通知' }}</span>
          </div>
...
const notifications = computed(() => notificationStore.getNotifications('subscribe'))
const loading = computed(() => notificationStore.isLoading('subscribe'))
const hasNext = computed(() => notificationStore.hasMore('subscribe'))
...
const load = (reset = false) => {
  notificationStore.fetchNotifications('subscribe', reset)
}
