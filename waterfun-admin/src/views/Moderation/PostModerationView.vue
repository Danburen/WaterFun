...
const renderedContent = (task: AuditResponsePostAuditPayload): string => {
  if (task.payload?.contentHtml) return task.payload.contentHtml;
  const raw = task.payload?.content || task.payload?.summary || '';
  if (!raw) return '(无内容)';
...
@media (max-width: 768px) {
  ...
  .post-title-display { font-size: 16px; padding: 12px; }
  .post-content-display { max-height: 200px; font-size: 14px; }
}
</style>
