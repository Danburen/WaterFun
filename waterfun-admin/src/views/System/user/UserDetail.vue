  ...
  <div v-loading="loading">
    <CardContainer title="user.detail">
      ...
      <el-descriptions v-if="detail" :title="t('user.basicInfo')" :column="2" border>
        ...
        <el-descriptions-item :label="t('user.nickname')">{{ detail.info.nickname || t('none.title') }}</el-descriptions-item>
        ...
        <el-descriptions-item :label="t('create.time')">{{ formatISOData(detail.info.createdAt) }}</el-descriptions-item>
        <el-descriptions-item :label="t('update.time')">{{ formatISOData(detail.info.updatedAt) }}</el-descriptions-item>
      ...
      <el-descriptions v-if="detail" :title="t('user.profileInfo')" :column="2" border class="section-gap">
        <el-descriptions-item :label="t('user.bio')">{{ detail.profile.bio || t('none.title') }}</el-descriptions-item>
        <el-descriptions-item :label="t('user.gender')">{{ detail.profile.gender || t('none.title') }}</el-descriptions-item>
        <el-descriptions-item :label="t('user.birthDate')">{{ detail.profile.birthDate || t('none.title') }}</el-descriptions-item>
        <el-descriptions-item :label="t('user.residence')">{{ detail.profile.residence || t('none.title') }}</el-descriptions-item>
      </el-descriptions>

      <el-descriptions v-if="detail" :title="t('user.contactInfo')" :column="2" border class="section-gap">
        <el-descriptions-item :label="t('user.email')">{{ detail.maskedData.emailMasked || t('none.title') }}</el-descriptions-item>
        <el-descriptions-item :label="t('user.phone')">{{ detail.maskedData.phoneMasked || t('none.title') }}</el-descriptions-item>
      </el-descriptions>
        ...
        <el-collapse v-model="collapseActive">
          <el-collapse-item :title="t('role.title')" name="roles">
            <el-row :gutter="10">
              ...
              <el-col v-if="detail.roles.length === 0" :span="24">{{ t('none.description') }}</el-col>
            </el-row>
          </el-collapse-item>
          <el-collapse-item :title="t('permission.title')" name="permissions">
            <el-row :gutter="10">
              ...
              <el-col v-if="detail.permissions.length === 0" :span="24">{{ t('none.description') }}</el-col>
            </el-row>
          </el-collapse-item>
        </el-collapse>
      </CardContainer>
