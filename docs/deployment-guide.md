...
```
               ...
               ┌────────┴─────────┐
               │                  │
       api.waterfun.top    admin.waterfun.top
               │                  │
          Gateway 容器        Nginx 容器
         (JWT 认证)        (Admin SPA 静态文件)
         ↙         ↘
...
| `docker-compose.app.yml` | nginx-proxy, acme-companion, admin-nginx, 后端服务 | 随版本更新 |
| `docker-compose.deploy.yml` | 仅后端服务（CI/CD 用，引用 GHCR 镜像） | CI/CD 自动更新 |
